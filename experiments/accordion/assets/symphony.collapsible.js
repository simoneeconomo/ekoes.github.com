...
(function($) {
	...
	/*-----------------------------------------------------------------------*/
		...
		/*-------------------------------------------------------------------*/
			...
			// Expand item
			object.on('expand.collapsible', settings.items, function(event, multiple) {
				var item = $(this),
					content = item.find(settings.content);
				...
				if(settings.accordion){
					if(!multiple)
						object.find(settings.items).not(item).trigger('collapse.collapsible');
				}
			});

			// Toggle single item
			object.on('click.collapsible keydown.collapsible', settings.handles, function(event) {
				var handle = $(this),
					item = handle.parents(settings.items);
					
				// Expand
				if(item.is('.collapsed')) {
					item.trigger('expand.collapsible', [(event.shiftKey)]);
				}
			...
			object.on('click.collapsible', 'a.collapser', function(event) {
				...
				// Expand all
					...
					items.trigger('expand.collapsible', true);
				}
			});
